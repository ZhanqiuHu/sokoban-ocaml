...
(* 
(* better change on_exit to on_hole *)
...
(* [get_player_pos room] is the pos of tile in [room] that is of ttype Player. 
*)
let get_player_pos room = 
  let player_tile = 
  room.map_tile_list |> List.flatten |> List.filter (fun tile  -> 
    match tile.ttype with 
    | Player _ -> true
    | _ -> false
    )
    |> List.hd
  in player_tile.position


let get_room_by_id room_id st = 
  st.all_rooms |> List.filter (fun room -> room.room_id = room_id) |> List.hd
...
let get_tile_by_loc loc room =     
  room.map_tile_list |> List.flatten |> List.filter 
  (fun tile -> tile.position = loc) |> List.hd
...
let update_loc_tile_to_player loc tile_list : tile list = 
  ...
  | [] -> []
  | h :: t -> (if h.position = loc 
    then {h with ttype = Player {on_exit = false}}
  else h ) :: t
...
let update_loc_tile_to_normal loc tile_list : tile list = 
  ...
  | [] -> []
  | h :: t -> (if h.position = loc 
    then {h with ttype = Normal {is_hole = false}}
  else h ) :: t
...
let list_to_nested_list room_width room_height lst = 
  let rec loop_helper lst room_width n = 
    match n with 
    | 0 -> []
    | _ -> List.filteri (fun i item -> i/room_width=room_height-n) lst 
    :: loop_helper lst room_width (n-1) 
  in (loop_helper lst room_width room_height)


let update_room_in_rm_list updated_room st : room list = 
  match st.all_rooms with 
  | [] -> []
  | h :: t -> (if h.room_id = st.current_room_id then updated_room 
  else h) :: t
...
let move (st : state) (dir: direction) : state = 
  ...
  if reach_boundary new_loc current_rm then st
  else 
    let updated_room = {current_rm with map_tile_list =
      current_rm.map_tile_list
      |> List.flatten
